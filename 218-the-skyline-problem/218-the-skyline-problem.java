//saw solution
//didn't have much time to solve T.T
class Solution {
...
class Solution {
    public List<List<Integer>> getSkyline(int[][] buildings) {
        ...
        for (int[] building : buildings) {
            buildings.add(List.of(building[0],building[1]));
        }
        
        int changed = 0;
        do {
            changed = 0;
            List<List<Integer>> newBuildings = new ArrayList();
            for (int i=0; i<buildings.size()-1; i++) {
                if (buildings.get(i).get(2) == buildings.get(i+1).get(2)) {
                    newBuildings.add(List.of(buildings.get(i).get(0),
                                             buildings.get(i+1).get(1),
                                             buildings.get(i).get(2)));
                }
                if (buildings.get(i).get(1) > buildings.get(i+1).get(0)) {
